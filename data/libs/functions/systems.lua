...
local function applyOutfitFor(caster, outfit, ms, subId, baseColors)
	if not caster or caster:isRemoved() then
		return
	end
...
local function applyFreezeFor(caster, ms, penalty)
	if not caster or caster:isRemoved() or ms <= 0 then
		return
	end
...
function createSpellAnimation(config)
	assert(type(config) == "table" and type(config.frames) == "table" and #config.frames > 0, "[createSpellAnimation] config.frames inválido")
	...
	return function(player, variant)
		...
		if ACTIVE[uid] then
			...
			local p = Player(uid)
			if p then
				p:removeCondition(CONDITION_OUTFIT, prev.subId)
			end
		end
		...
		for i = 1, math.min(#frames, MAX_FRAMES) do
			...
			safePlayerEvent(uid, startAt, function(caster)
				...
				if f.say then
					caster:say(f.say, TALKTYPE_MONSTER_SAY)
				end
				...
				if type(f.cast) == "function" then
					...
					if not ok then
						print("[SpellAnim] cast error:", err)
					end
				end
			end)
...
function cancelSpellAnimation(player)
	...
	if state then
		local p = Player(uid)
		if p then
			p:removeCondition(CONDITION_OUTFIT, state.subId)
		end
		ACTIVE[uid] = nil
	end
end
