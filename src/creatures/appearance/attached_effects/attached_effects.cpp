...
bool AttachedEffects::loadFromXml() {
	...
	for (auto outfitNode : doc.child("attachedeffects").children("outfit")) {
		outfits.push_back(std::make_shared<AttachedOutfit>(
...
std::shared_ptr<AttachedOutfit> AttachedEffects::getOutfitByID(uint8_t id) {
	auto it = std::ranges::find_if(outfits.begin(), outfits.end(), [id](const std::shared_ptr<AttachedOutfit> &outfit) {
		return outfit->id == id;
	});
...
std::shared_ptr<AttachedOutfit> AttachedEffects::getOutfitByName(const std::string &name) {
	auto outfitName = name.c_str();
	auto it = std::ranges::find_if(outfits.begin(), outfits.end(), [outfitName](const std::shared_ptr<AttachedOutfit> &outfit) {
